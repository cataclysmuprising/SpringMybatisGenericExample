...
import com.mycom.products.springMybatisGenericExample.core.api.CommonGenericTest;
...
import com.mycom.products.springMybatisGenericExample.core.bean.config.UserBean.Gender;
import com.mycom.products.springMybatisGenericExample.core.service.config.api.UserService;
import com.mycom.products.springMybatisGenericExample.core.util.Cryptographic;
...
public class UserFunctionalTest extends TestBase implements JoinedSelectableTest, CommonGenericTest {
	...
	public void testSelectCountWithLazyCriteria() throws Exception {
		...
		long count = userService.selectCounts(criteria, FetchMode.LAZY);
		testLogger.info("Total counts ==> " + count);
		Assert.assertEquals(true, count > 0);
	}
	...
	public void testSelectCountWithEagerCriteria() throws Exception {
		...
		// criteria.put("gender", Gender.MALE);
		// criteria.put("word", "user");
		long count = userService.selectCounts(criteria, FetchMode.EAGER);
		testLogger.info("Total counts ==> " + count);
		Assert.assertEquals(true, count > 0);
	}
	...
	public void testSelectMultiRecordByCriteriaWithLazyMode() throws Exception {
		...
		List<UserBean> results = userService.selectList(criteria, FetchMode.LAZY);
		Assert.assertNotNull(results);
		Assert.assertEquals(true, results.size() > 0);
		showEntriesOfCollection(results);
	}
	...
	public void testSelectMultiRecordByCriteriaWithEagerMode() throws Exception {
		...
		criteria.put("orderBy", "loginDate");
		criteria.put("orderAs", "desc");
		List<UserBean> results = userService.selectList(criteria, FetchMode.EAGER);
		Assert.assertNotNull(results);
		Assert.assertEquals(true, results.size() > 0);
		showEntriesOfCollection(results);
	}
	...
	public void testInsertSingleRecord() throws Exception {
		...
		long lastInsertedId = userService.insert(user, TEST_CREATE_USER_ID);
	...
	public void testInsertMultiRecords() throws Exception {
		...
		records.add(record2);
		userService.insert(records, TEST_CREATE_USER_ID);
	}
	...
	public void testSingleRecordUpdate() throws Exception {
		...
		user.setDob(LocalDate.now());
		user.setAddress("California");
		long totalEffectedRows = userService.update(user, TEST_UPDATE_USER_ID);
	...
	public void testUpdateByCriteria() throws Exception {
		...
		userService.update(criteria, updateItems, TEST_UPDATE_USER_ID);
	}
	...
	public void testDeleteByCriteria() throws Exception {
		...
		long totalEffectedRows = userService.delete(criteria, TEST_UPDATE_USER_ID);
		Assert.assertEquals(true, totalEffectedRows > 0);
		testLogger.info("Total effected rows = " + totalEffectedRows);
	}
